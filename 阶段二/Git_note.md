...
第一次使用 ***clone*** 或者 ***push*** 时有 SSH 警告,验证key的信息是否源于 github 确认后输入 yes 就行了
...
> 两个版本库在没有各自都有新的提交的时候,默认使用快速合并,在这种模式下删除分支后会丢失分支信息,在历史上看不到分支信息
